...
import { InsertionType, TYPE_TASK_DROP_SPACE, TYPE_TASK } from '../config/dndConstants';
    ...
    accept: (source) => {
      if (source.type === TYPE_TASK) {
        if (insertionType === InsertionType.before) {
          if (source.id !== nextTaskId && source.id !== prevTaskId) return true;
        }

        if (insertionType === InsertionType.append) {
          if (source.id !== prevTaskId) return true;
        }
      }
