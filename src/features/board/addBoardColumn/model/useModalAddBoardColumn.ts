...
export type UseModalAddBoardColumnProps = { board: Board };

export const useModalAddBoardColumn = ({ board }: UseModalAddBoardColumnProps) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const boardDraft = {
        ...board,
        columns: [...board.columns, { title: values.title, tasksIds: [] }],
      };
      const result = await addBoardColumn(board.id, boardDraft, () => setFormError(''));
